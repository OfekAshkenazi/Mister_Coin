...
export class ContactService {
    ...
    private _contactFilter$ = new BehaviorSubject<ContactFilter>({ name: '', email: '', phone: '' });
    ...
    private _addContact(contact: Contact) {
        const newContact = new Contact(contact.name, contact.email, contact.phone, contact.imgUrl);
    ...
    private _filter(contacts: Contact[], filterBy: ContactFilter) {
        if (filterBy.name) {
            ...
            return contacts.filter(contact => {
                return contact.phone.toLocaleLowerCase().includes(filterBy.phone.toLocaleLowerCase())

            })
        } else if (filterBy.email) {
            return contacts.filter(contact => {
                return contact.email.toLocaleUpperCase().includes(filterBy.email.toLocaleUpperCase())
